...
def color_cells(df) :
    ...
    for td in s.findAll('td') :
        k = td.string
        k = str(255-int(float(td.string)*255))
        td['style'] = 'background-color: rgb(255,'+k+','+k+')'
    return unicode(s.prettify('utf8'))


#path = '/data/mallet_tests/from_mallet/thursday_compostion.txt'
def df2html(input_path, dest_path) :
    mega_list = pd.read_csv(input_path, delimiter='\t', names=['year','File']+[str(i) for i in range(50)], na_values=[''])

    mega_list.File = mega_list.File.str.split('/').str[-1]
