...
def xml_clean(s, stem, opt) :
    ...
    s = re.sub('[^a-z]', ' ', re.sub(r'-\s+', '', s.text.lower().strip()))
...
def xml_open(in_file, dest_path, opt, stem) :
    ...
    try :
        ...
        if opt == 'split' :
            ...
            print make_sec_files(dest_path, article, a, s, stem, opt)
...
def make_sec_files(dest_path, article, abstr, soup, stem, opt) :
    ...
    def double_act(sec) :
        return xml_clean(sec,stem,opt)

    def save_secs(s, sec_len, title, sec_num, i) :
        global title_list
        dest_file = dest_path+'/'+article+'/'+article+'_'+str(sec_num+i)+'.txt'
        
        if sec_len > 10 :
            title_list += article+'_'+str(sec_num+i)+','+title+'\n'
            try :
    ...
    if abstr != None:
        a = xml_clean(abstr,stem,'full')
        secs.insert(0, ['abstr',a])

    print type(secs)
    
    map(lambda x : save_secs(x[1], len(x[1].split()), x[0], secs.index(x), i), secs)
